...
This document defines the documentation boundaries of `docs/05-life-os/`
— exactly what belongs inside this folder, and what does not — before any
Life OS documentation is authored.

# Why This Repository Exists

`docs/05-life-os/` exists because `XP-0014` §9 anticipates a Domain
Architecture document for Life OS as one of the five major architectural
domains, and `XP-0030` establishes the constitutional contract that
document must satisfy once authored. This folder is the designated
location where that responsibility is discharged. It answers only why
this folder exists as a documentation location — not why Life OS itself
exists as a product, which is Life OS content and out of scope for this
document.

# Repository Scope

This folder holds documentation that describes Life OS, and only Life
OS, as one Digital Operating System instance:

- The Life OS Domain Architecture document, once authored, satisfying
...
- Any Module Architecture, Feature Specification, or Implementation Plan
  ...
  recognizes as subordinate to Life OS's own Domain Architecture, once
  each is actually authorized to be written.
- Internal, non-authoritative planning material for sequencing Life OS's
...
- Any other folder's own subject matter, including `03-business-os/`,
  `04-community-os/`, `06-marketplace/`, `07-payments/`, `08-ai/`, or any
...
- **Life OS Domain Architecture** — the single document satisfying
  `XP-0030`'s contract for Life OS, anticipated by `XP-0014` §9.

# Current Status

`docs/05-life-os/` currently contains no authored Life OS documentation.
Its planned Domain Architecture document remains unnumbered and
