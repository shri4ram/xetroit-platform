...
This document defines the documentation boundaries of `docs/04-community-os/`
— exactly what belongs inside this folder, and what does not — before any
Community OS documentation is authored.

# Why This Repository Exists

`docs/04-community-os/` exists because `XP-0014` §9 anticipates a Domain
Architecture document for Community OS as one of the five major
...
Community OS itself exists as a product, which is Community OS content
and out of scope for this document.

# Repository Scope

This folder holds documentation that describes Community OS, and only
Community OS, as one Digital Operating System instance:

- The Community OS Domain Architecture document, once authored,
  satisfying the contract `XP-0030` establishes for every Digital
  Operating System.
- Any Module Architecture, Feature Specification, or Implementation Plan
  ...
  recognizes as subordinate to Community OS's own Domain Architecture,
  once each is actually authorized to be written.
- Internal, non-authoritative planning material for sequencing Community
...
- **Community OS Domain Architecture** — the single document satisfying
  `XP-0030`'s contract for Community OS, anticipated by `XP-0014` §9.

# Current Status

`docs/04-community-os/` currently contains no authored Community OS
